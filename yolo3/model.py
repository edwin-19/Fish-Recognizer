...
def darknet_body(x):
    '''Darknet body having 52 convolution 2d layers'''
...
def yolo_correct_boxes(box_xy, box_wh, input_shape, image_shape):
    ...
    # scale boxes back to original iamge shape
...
def yolo_eval(yolo_outputs,
    ...
    for i in range(num_layers):
        _boxes, _box_scores = yolo_boxes_and_scores(
            yolo_outputs[i], anchors[anchor_mask[i]],
...
def yolo_loss(args, anchors, num_classes, ignore_thresh=.5, print_loss=False):
    ...
    for l in range(num_layers):
        ...
        pred_box = K.concatenate(
            [pred_xy, pred_wh]
        )
        
        # Darkent raw box to calculate loss
